...
import { AddTeamDialog } from "../dialog/addDeleteTeam.project";
import { useRouter } from "next/navigation";
import { useAuth } from "@/components/auth-provider";
import { EmptyState } from "@/components/common/data-display/EmptyState";
import { Skeleton } from "@/components/atomics/Skeleton.atomic";
import { RemoveTeamDialog } from "../dialog/addDeleteTeam.project";
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Teams participating in this project
          </p>
        </div>
        {userRole === "admin" && (
          <Button
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-xs sm:text-sm mt-2">
                      ...
                      <div className="text-center sm:text-left">
                        ...
                        <div className="font-medium">
                          {formatDate(team.updatedAt?.toDate())}
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {deleteDialogOpen && (
        <RemoveTeamDialog
          ...
          teamName={
            teams.find((t) => t.id === deleteDialogOpen)?.name ?? undefined
          }
        />
      )}
    </div>
