...
export default function AddUserDialog({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!displayName.trim() || !email.trim() || !role || !password) {
      toast.error(
        "Please fill in all required fields: Display Name, Email, Password, and Role."
      );
      return;
    }
    ...
    try {
      if (secondaryAuth) {
        try {
          ...
          console.warn(
            "Secondary auth failed, falling back to main auth:",
            secondaryAuthError
          );
      ...
      const usedSecondaryAuth =
        secondaryAuth && !localStorage.getItem("tempFallbackToMainAuth");
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="py-2">
              User{" "}
              <span className="font-semibold">{newUserCreatedInfo.name}</span> (
              {newUserCreatedInfo.email}) has been successfully created.
              <br />
              <br />
              For security reasons, you have been logged out. Please log back in
              with your admin credentials to continue managing users.
            </DialogDescription>
          </DialogHeader>
    ...
    >
      <DialogContent className="sm:max-w-[525px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Manually add a new user to the system. This will create both an
            authentication account and a user record.
          </DialogDescription>
        </DialogHeader>
