...
export default function EditUserDialog({
  ...
  useEffect(() => {
    if (userToEdit && isOpen) {
      ...
      setBaseSalary(
        userToEdit.baseSalary === undefined ? "" : userToEdit.baseSalary
      );
    } else if (!isOpen) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!displayName.trim() || !role || !status) {
      toast.error(
        "Please fill in all required fields: Display Name, Role, and Status."
      );
      return;
    }
      ...
      > = {
        ...
        baseSalary:
          typeof baseSalary === "number"
            ? baseSalary
            : parseFloat(baseSalary as string) || 0,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px]">
        <DialogHeader>
          <DialogTitle>
            Edit User: {userToEdit?.displayName || userToEdit?.email}
          </DialogTitle>
          <DialogDescription>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value: string) =>
                  setStatus(value as UserData["status"])
                }
                required
              >
                <SelectTrigger className="col-span-3">
