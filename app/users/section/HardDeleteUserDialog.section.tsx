...
import {
  deleteUserRecord, // Fungsi baru dari firestore.ts
  addActivity,
  ActivityActionType,
  type UserData,
} from "@/lib/firestore";
...
export default function HardDeleteUserDialog({
  ...
  const handleHardDelete = async () => {
    ...
    if (adminUser.uid === userToDelete.id && userToDelete.role === "admin") {
      toast.error(
        "Admin cannot permanently delete their own account through this dialog."
      );
      onOpenChange(false);
      return;
    }
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="py-2 text-base">
            ...
            <br />
            <br />
            This action is irreversible and will remove the user's data from the
            application's database (Firestore).
            <br />
            <br />
            <strong className="text-red-700">
              Important: Deleting the user from Firebase Authentication (if not
              handled by a separate backend process) must be done manually via
              the Firebase Console to prevent an orphaned authentication
              account.
            </strong>
            <br />
            <br />
