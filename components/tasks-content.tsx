...
export function TasksContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedTasks = await getTasks(
        userRole === "user" ? user.uid : undefined
      );
  ...
  const handleEditTask = (task: Task) => {
    ...
    setFormData({
      ...
      deadline: task.deadline
        ? new Date(task.deadline.toDate()).toISOString().split("T")[0]
        : "",
  ...
  // Memoized project map for performance optimization
  ...
  // Memoized user map for performance optimization
  ...
  // Helper function to get user names from user IDs array
  const getUserNames = (userIds: string | string[] | undefined): string => {
    if (!userIds) return "Unassigned";

    // Handle single string ID
    if (typeof userIds === "string") {
      return userMap[userIds] || "Unknown User";
    }

    // Handle array of IDs
    if (Array.isArray(userIds)) {
      if (userIds.length === 0) return "Unassigned";
      return userIds.map((id) => userMap[id] || "Unknown User").join(", ");
    }
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[525px]">
              <form onSubmit={handleSubmit}>
                ...
                <DialogFooter>
                  <Button type="submit" aria-label="Create new task">
                    Create Task
                  </Button>
                </DialogFooter>
              </form>
              ...
              <TableBody>
                {tasks.map((task) => (
                  <TableRow key={task.id}>
                    ...
                    {userRole === "admin" && (
                      <TableCell>{getUserNames(task.assignedTo)}</TableCell>
                    )}
